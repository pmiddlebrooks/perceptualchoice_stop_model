function ds = plot_fits_go(subject,model,architecture,dt,trialVar,fileStr,responseSide, accuracy, savePlot)
    ...
    function plotit(SAM,prd,p,model,iModel,modelStr,cost,altCost, responseSide, accuracy)
        ...
        if strcmp(accuracy, 'both') || strcmp(accuracy, 'correct')
            ...
            for iResponse = 1:length(responseArray)
                for iCondition = 1:length(conditionArray);
                    ...
                    if strcmp(accuracy, 'both') || strcmp(accuracy, 'correct')
                        ...
                        obsRT = (obs.rtGoCCorr{iTrialCatGo});
                         obsRT = sort(obsRT);
            ...
            for iRT = min(obsRT) : max(obsRT)
             ...
             plot(min(obsRT):max(obsRT), iObsGoRT,'color', 'r', 'lineWidth', 2)
      end
                end
