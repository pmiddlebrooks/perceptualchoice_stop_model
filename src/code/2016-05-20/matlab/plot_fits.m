function plot_fits(subject,model,architecture,dt,trialVar,optimScope,fileStr,responseSide, accuracy, defective, savePlot)
...
for iSubject = 1:nSubject
    ...
    function plotit(SAM,prd,p,model,iModel,modelStr,cost,altCost, responseSide, accuracy, defective)
        ...
        ntiles                  = SAM.optim.cost.stat.cumProb;
        ...
        if strcmp(optimScope, 'stop') || strcmp(optimScope, 'all')
            ...
            for iResponse = 1:length(responseArray)
                for iCondition = 1:length(conditionArray);
                    ...
                    if defective
                        cumPStopIErrorCCorrPrd   = cumPStopIErrorCCorrPrd * prd.pStopIErrorCCorr(iTrialCatGo);
%                         if sum(~isnan(cumPStopIErrorCCorrPrd)) == 0
%                             cumPStopIErrorCCorrPrd = []
%                         end
                        cumPStopIErrorCCorrObs   = cumPStopIErrorCCorrObs * obs.pStopIErrorCCorr(iTrialCatGo);
