function job_spec_x0base(subj,dt,trialVar,optimScope,choiceMech,stopMech,iModel,fileStr,x0Base,doPlot,doSave,doStartParCluster)
%
% INPUTS
...
if doStartParCluster
    ...
    load(fullfile(sprintf(rootDir,subj(1),dt,trialVarStr,modelArch),sprintf(nameSAMGeneral,optimScope)));
...
for iSubjInd = 1 : length(subj)
    ...
    % =========================================================================
    load(fullfile(sprintf(rootDir,iSubj,dt,trialVarStr,modelArch),sprintf(nameSAMGeneral,optimScope)));
    ...
    switch optimScope
        ...
        case 'stop'
            ...
            ds = dataset('File',fullfile(sprintf(rootDir,iSubj,dt,trialVarStr,modelArchGO),sprintf(bestXGO,iModel)),'HeaderLines',0);
    ...
    % =========================================================================
    if doSave
        save(fullfile(sprintf(rootDir,iSubj,dt,trialVarStr,modelArch),sprintf(nameUserSpecX,optimScope,iModel)),'-ascii','-double','-tabs','X');
    end
