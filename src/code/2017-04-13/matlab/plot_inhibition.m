...
p = panel();
p.pack({.1 .25 .25 .25 .15}, num2cell(repmat(1/4,1,4)));
        ...
        for iResponse = 1:length(responseArray)
            for iCondition = 1:length(conditionArray);
                ...
                for kSSD = 1 : length(iSsdArray)
                    ...
                    if obs.nStopICorr(kTrialCatStop) >= minTrialCond
                    ...
                    % Plot sets of actiavtion funciton means:
                    if ~isnan(cancelTime(kTrialCatStop))
                        switch subject
                            case 1
                                yMax = 100;
                        ...
                        clf
                        hold 'all'
                        plot(kGoStopAct(1:xMax), 'r')
                        plot(kGoSlowAct(1:xMax), 'g')
                        
                        plot([cancelTime(kTrialCatStop) cancelTime(kTrialCatStop)], [0 yMax], '--k')
                        plot([iSsdArray(kSSD) iSsdArray(kSSD)], [0 yMax], 'r')
                        plot([ssrtModel(kTrialCatStop)+iSsdArray(kSSD), ssrtModel(kTrialCatStop)+iSsdArray(kSSD)], [0 yMax], 'b')
                        pause
                        figure(mainPlots)
                    end
