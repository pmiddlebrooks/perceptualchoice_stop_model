...
trialData = load(fullfile(rawDataDir,[subject,'_',sessions,'.mat']));
SessionData = trialData.SessionData;
trialData = rmfield(trialData, 'SessionData');

% % Remove trials with super short RTs
% % =========================================================================
% rtMin         = 100; % minimum # of stop trials for a SSD to be included
% truncateRT     = true;
% 
% if truncateRT
%     
%     trialData(trialData.rt > 0 & trialData.rt < rtMin,:) = [];
% end


% Concatentate SSDs 1 ms from each other
% =========================================================================
ssdList = unique(trialData.ssd(~isnan(trialData.ssd), :));
a = diff(ssdList);
changeSSD = ssdList(a == 1);
for i = 1 : length(changeSSD)
    trialData.ssd(trialData.ssd == changeSSD(i)) = trialData.ssd(trialData.ssd == changeSSD(i)) + 1;
end
ssdList = unique(trialData.ssd(~isnan(trialData.ssd), :));

% Remove SSDs with low trial numbers?
% =========================================================================
% nSSDMin         = 15; % minimum # of stop trials for a SSD to be included
% truncateSSD     = true;
% 
% if truncateSSD
%     
...
% Remove 50% color coherence trials?
% =========================================================================
remove50 = true;
if remove50
    trialData = structfun(@(x) x(trialData.targ1CheckerProp ~= .5), trialData,'uni',false);
end


nObs = size(trialData.sessionTag,1);
...
% iGoCCorr = zeros(nObs, 1);
% iGoCCorrTrial          = ccm_trial_selection(trialData, opt);
% iGoCCorr(iGoCCorrTrial) = 1;
% iGoCCorr = logical(iGoCCorr);
% 
% opt.outcome = {'goCorrectDistractor', 'distractorHoldAbort'};
% iGoCError = zeros(nObs, 1);
% iGoCErrorTrial         = ccm_trial_selection(trialData, opt);
% iGoCError(iGoCErrorTrial) = 1;
% iGoCError = logical(iGoCError);

iGoCCorr = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'goCorrectTarget','once')),trialData.trialOutcome,'Uni',0)) | ...
    (cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'targetHoldAbort','once')),trialData.trialOutcome,'Uni',0)) & ...
    isnan(trialData.ssd));

iGoCError = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'goCorrectDistractor','once')),trialData.trialOutcome,'Uni',0)) | ...
    (cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'distractorHoldAbort','once')),trialData.trialOutcome,'Uni',0)) & ...
    isnan(trialData.ssd));

% iGoCCorrO          = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'goCorrectTarget','once')),trialData.trialOutcome,'Uni',0));
% iGoCErrorO         = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'goCorrectDistractor','once')),trialData.trialOutcome,'Uni',0));

% Stop Trials
% opt.ssd = 'collapse';
% 
% opt.outcome = {'stopCorrect'};
% iStopICorr = zeros(nObs, 1);
% iStopICorrTrial        = ccm_trial_selection(trialData, opt);
% iStopICorr(iStopICorrTrial) = 1;
% iStopICorr = logical(iStopICorr);
% 
% opt.outcome = {'stopIncorrectTarget', 'targetHoldAbort'};
% iStopIErrorCCorr = zeros(nObs, 1);
% iStopIErrorCCorrTrial  = ccm_trial_selection(trialData, opt);
% iStopIErrorCCorr(iStopIErrorCCorrTrial) = 1;
% iStopIErrorCCorr = logical(iStopIErrorCCorr);
% 
% opt.outcome = {'stopIncorrectDistractor', 'distractorHoldAbort'};
% iStopIErrorCError = zeros(nObs, 1);
% iStopIErrorCErrorTrial = ccm_trial_selection(trialData, opt);
% iStopIErrorCError(iStopIErrorCErrorTrial) = 1;
% iStopIErrorCError = logical(iStopIErrorCError);
...
% iStopICorrO        = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'stopCorrect','once')),trialData.trialOutcome,'Uni',0));
% iStopIErrorCCorrO  = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'stopIncorrectTarget','once')),trialData.trialOutcome,'Uni',0));
% iStopIErrorCErrorO = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'stopIncorrectDistractor','once')),trialData.trialOutcome,'Uni',0));

iCorr             = iGoCCorr | iStopICorr;

% Aborted trials
iAbort            = cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'fixationAbort$','once')),trialData.trialOutcome,'Uni',0)) | ...
    cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'sacadeAbort$','once')),trialData.trialOutcome,'Uni',0)) | ...
    cell2mat(cellfun(@(in1) ~isempty(regexp(in1,'choiceStimulusAbort$','once')),trialData.trialOutcome,'Uni',0));
...
switch subject
    case 'broca'
        ...
        allData.cnd(trialData.targ1CheckerProp == 0.42 | ...
            trialData.targ1CheckerProp == 0.58)       = 1;
        % Hard
        allData.cnd(trialData.targ1CheckerProp == 0.47 | ...
            trialData.targ1CheckerProp == 0.53)       = 2;
    case 'joule'
        % Easy
        allData.cnd(trialData.targ1CheckerProp == 0.40 | ...
            trialData.targ1CheckerProp == 0.60)       = 1;
        % Hard
        allData.cnd(trialData.targ1CheckerProp == 0.44 | ...
            trialData.targ1CheckerProp == 0.45 | ...
            trialData.targ1CheckerProp == 0.55 | ...
            trialData.targ1CheckerProp == 0.56)       = 2;
    otherwise
        error('Need to enter color coherence values for this subject, job_preproc_data line 193')
end
% Primary stimulus (go-signal) index
% =========================================================================
allData.stm1(trialData.targ1CheckerProp < 0.5) = 1;
allData.stm1(trialData.targ1CheckerProp > 0.5) = 2;
...
allData.resp(iTarget0   & iGoCError)            = 1;

allData.resp(iTarget180 & iGoCCorr)             = 1;
allData.resp(iTarget180 & iGoCError)            = 2;

allData.resp(iTarget0   & iStopIErrorCCorr)     = 2;
allData.resp(iTarget0   & iStopIErrorCError)    = 1;

allData.resp(iTarget180 & iStopIErrorCCorr)     = 1;
allData.resp(iTarget180 & iStopIErrorCError)    = 2;

% Response time
% =========================================================================
allData.rt(:)        = trialData.responseOnset - trialData.responseCueOn;
allData.rt(iStopICorr) = 0;
...
% 4.b. If fitting only correct choice trials, REMOVE CHOICE ERROR TRIALS
errorTrial = iGoCError | iStopIErrorCError;

% Remove aborted and non-fixated trials
% =========================================================================
abortTrial = iAbort | iNoFix | iOmit;

% Remove express saccades and unacceptably long saccades
% =========================================================================
saccRemove = (allData.rt > 0 & allData.rt < 100) | allData.rt > 2000;


allData(abortTrial | saccRemove,:) = [];
